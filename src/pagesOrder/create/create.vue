...
const selectAddress = computed(() => {
  return addressStore.selectedAddress || preOrder.value?.userAddresses.find((v) => v.isDefault)
})
// 提交
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="settlement">
      <view class="item">
        ...
        <text class="number symbol">{{ preOrder?.summary.totalPayPrice.toFixed(2) }}</text>
      </view>
      <view class="item">
        <text class="text">运费: </text>
        <text class="number symbol">{{ preOrder?.summary.postFee.toFixed(2) }}</text>
      </view>
    </view>
  </scroll-view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    <view class="total-pay symbol">
      <text class="number">{{ preOrder?.summary.totalPayPrice.toFixed(2) }}</text>
    </view>
