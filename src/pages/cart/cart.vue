...
import { delectMemberCartApi, getMemberCartApi, putMemberCartApi } from '@/services/cart'
...
// 点击删除调用
const onDelectCart = (skuId: string) => {
  uni.showModal({
    content: '是否删除',
    success: async ({ confirm }) => {
      // 删除
      confirm && (await delectMemberCartApi({ ids: [skuId] }))
      // 重新获取列表
      getMemberCartData()
    },
  })
      ...
      <view class="cart-list" v-if="cartList.length">
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <!-- 右侧删除按钮 -->
            <template #right>
              <view class="cart-swipe-right" @tap="onDelectCart(item.skuId)">
